...
const ChartsPage = () => {
  ...
  const onTreeSubmit = async () => {
    ...
    const populationPerAgeJsons = (regionsIDs as string[]).map(async (id) =>
    ...
    const ruralToCityRatioPerAgeJsons = (regionsIDs as string[]).map(
      async (id) =>
        (
          await fetch(`http://localhost:3002/ruralToCityRatioPerAge?key=${id}`)
        ).json()
    );
    let ruralToCityRatioPerAgeData = await Promise.all(
      ruralToCityRatioPerAgeJsons
    );
    ...
    const womenToMenRatioPerAgeJsons = (regionsIDs as string[]).map(
      async (id) =>
        (
          await fetch(`http://localhost:3002/womenToMenRatioPerAge?key=${id}`)
        ).json()
    );
    let womenToMenRatioPerAgeData = await Promise.all(
      womenToMenRatioPerAgeJsons
    );
  ...
  const messageWrapper = (
    <MessageWrapper
      delay={400}
      fetchAddress="http://localhost:3002/subjectTree"
      loadingMessageContent="Загружаю список субъектов РФ..."
      errorMessageContent="Не удалось загрузить субъекты РФ"
    ...
    <div className="charts-with-tree">
      <div className="tree">
        <TreeSelect
          ...
          treeData={subjects}
        />
        <Button type="primary" onClick={onTreeSubmit} className="submit-tree">
          Submit
        </Button>
      </div>
