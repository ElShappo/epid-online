...
import { getMergedTablesData, getMergedRegionsName, getSubjectTree, getTotalPopulationPerAge } from './utils/excel';
...
app.get('/subjectsPopulationsByAges', async (req: Request, res: Response) => {
    let keys = req.query.key as string | string[]; // 2.1. corresponds to 'Центральный федеральный округ'
    console.log(`Initial keys: ${keys}`);
